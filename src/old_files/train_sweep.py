...
def parse_args():
    ...
    parser.add_argument('--lambda_adversarial', type=float, help='Lambda for adversarial training')
    parser.add_argument('--gradient_accumulation_steps', type=int, help='Gradient accumulation steps')
    parser.add_argument('--max_iters', type=int, help='Maximum iterations')
...
def init_model(config, meta_vocab_size, device):
    ...
    if config.init_from == 'scratch':
        ...
        # ALWAYS reset iteration counter in sweep runs to prevent the key issue
        # This avoids inheriting the large iteration count from the checkpoint
        iter_num = 0
        print(f"Original checkpoint iteration was: {checkpoint.get('iter_num', 'unknown')}")
        print(f"Reset iteration counter to 0 for this run")
        
        # Still track best val loss from checkpoint
...
def main():
    ...
    if config.wandb_log and config.master_process:
        ...
        # Define metrics - moved here to ensure they're set up before any logging
        wandb.define_metric("iteration")
        wandb.define_metric("*", step_metric="iteration")
        
        # Get sweep parameters from wandb.config
        sweep_params = dict(wandb.config)
        sweep_params.pop('program', None)
        sweep_params.pop('command', None)
        
        # Debug: Print sweep parameters
        print("\n=== SWEEP PARAMETERS ===")
        for key, value in sweep_params.items():
            print(f"Sweep param: {key} = {value}")
        print("=======================\n")
        
        # Create unique directory name using sweep parameters
        unique_dir = f"lambda_{sweep_params.get('lambda_adversarial', 'default')}_" \
                     f"accum_{sweep_params.get('gradient_accumulation_steps', 'default')}_" \
                     f"iters_{sweep_params.get('max_iters', 'default')}"
        
        # Update config with sweep parameters
        for key, value in sweep_params.items():
            if hasattr(config, key):
                setattr(config, key, value)
                print(f"Updated {key} to {value} from sweep config")
                
            # Debug: Print final configuration values
            print("\n=== FINAL CONFIGURATION ===")
            print(f"max_iters: {config.max_iters}")
            print(f"eval_interval: {config.eval_interval}")
            print(f"lambda_adversarial: {config.lambda_adversarial}")
            print(f"train_model: {config.train_model}")
            print(f"train_probes: {config.train_probes}")
            print(f"train_adversarially: {config.train_adversarially}")
            print("==========================\n")
        
        # Update destination directory
        config.dest_dir = os.path.join(config.dest_dir, unique_dir)
        print(f"\nUpdated directory configuration:")
        print(f"Sweep parameters: {sweep_params}")
        print(f"Final dest_dir: {config.dest_dir}")
        
        # Update wandb run name to match directory
        wandb.run.name = unique_dir
        wandb.run.save()
    ...
    while True:
        ...
        if iter_num % config.eval_interval == 0 and config.master_process:
            ...
            if config.wandb_log:
                # IMPORTANT: Reset to use the actual iteration number from this run, not the step count
                # This is the key fix to the visualization issue
                log_dict = {
                    "iteration": iter_num,  # The actual iteration in this run
                ...
                # Use the x-value key that wandb expects for plotting
                wandb.define_metric("iteration")
                wandb.define_metric("*", step_metric="iteration")
                
                print(f"Logging to wandb at iteration {iter_num} (actual count)")
            ...
            if config.train_probes:
                ...
                if config.wandb_log:
                    for split in probe_eval_stats:
                        for stat in probe_eval_stats[split]:
                            for i in range(probe_cluster.get_num_probes()):
                                metric_name = f"{split}/probe-{stat}-{'attn' if i%2==0 else 'MLP'}-layer-{i//2}"
                                log_dict[metric_name] = probe_eval_stats[split][stat][i]
                                
                                # Debug: Print the metric being logged
                                print(f"Adding metric: {metric_name} = {probe_eval_stats[split][stat][i]:.4f}")
            
            # Log to wandb (only once)
            if config.wandb_log:
                # Make sure the step value is correct here!
                wandb.log(log_dict)
                print(f"Successfully logged to wandb at iteration {iter_num}")
                
                # DEBUG: Let's see what's in the log_dict
                print("Log dictionary contents:")
            ...
            if ((not config.never_save_checkpoint and losses['val'] < best_val_loss) or 
                ...
                if iter_num > 0:
                    ...
                    checkpoint_path = os.path.join(config.dest_dir, 'ckpt.pt')
                    
                    print(f"\nCheckpoint debug info:")
                    print(f"Base directory: {config.dest_dir}")
                    print(f"Full checkpoint path: {os.path.abspath(checkpoint_path)}")
                    print(f"Directory exists: {os.path.exists(os.path.dirname(checkpoint_path))}")
        ...
        if iter_num > config.max_iters or stop_requested:
            print(f"Terminating at iteration {iter_num} (max_iters={config.max_iters}, stop_requested={stop_requested})")
